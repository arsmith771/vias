...
add_image_size( 'post-image', 650, 650, false ); // width, height, crop
...
add_action('genesis_header', 'header_8', 8 );
function header_8(){

		echo '<div class="header-social-media"><a class="header-social-media__fb" href="' . types_render_field('facebook-page-address', array("id" => "5", "output" => "raw") ) . '" title="Visit us on Facebook (Link opens in a new tab or window)" target="_blank"></a><a class="header-social-media__twitter" href="' . types_render_field('twitter-account-address', array("id" => "5", "output" => "raw") ) . '" title="Visit us on Twitter (Link opens in a new tab or window)" target="_blank"></a></div>';
...
add_action('genesis_before_content', 'news_archive_header', 10 );
function news_archive_header(){
	if ( ( !is_front_page() && is_home() ) || is_archive() || is_singular( 'post' ) ) {
			echo '<div class="news-listing-header"></div>';
	}
}

// Add News Archive widget area 
genesis_register_sidebar( array(
	'id'		=> 'news-archive',
	'name'		=> __( 'New archive list', '' ),
	'description'	=> __( '', '' ),
) );

add_action('genesis_before_content', 'news_archive_listing', 11 );
function news_archive_listing(){
		if ( ( !is_front_page() && is_home() ) || is_archive() ) {
			genesis_widget_area ('news-archive', array(
        	'before' => '<div class="news-archive">',
        	'after' => '</div>',
		) );
	}
}
